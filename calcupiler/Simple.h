...
namespace Calcupiler
{
	namespace Simple
	{
		...
		public:
			...
			//######################################################
			static constexpr T Sum(const T lhs, const T rhs) {
				return lhs + rhs;
			}

			static constexpr T Diff(const T lhs, const T rhs) {
				return lhs - rhs;
			}

			static constexpr T Mul(const T lhs, const T rhs) {
				return lhs * rhs;
			}

			static constexpr T Div(const T lhs, const T rhs) {
				return lhs / rhs;
			}
			...
			static constexpr T Pow(T basis, E exp) {
				return std::is_integral_v<E> ? 
					Pow_i(basis, exp) : Pow_f(basis, exp);
			}
